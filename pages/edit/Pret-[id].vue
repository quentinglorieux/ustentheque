            ...
            <div class="">
              <div class="flex flex-wrap align-items-center justify-content-between gap-2">
                <div class="flex align-items-center gap-2">
                  <i class="pi pi-tag"></i>
                  <span> Marque:</span><span class="font-semibold">{{ resa.objet.marque }}</span>
                </div>
...
async function retrieveOneResa() {
  try {
    const publicData = await directus.request(readItem("reservation", route.params.id, {
      fields: [
        "id", "debut", "fin", "statut", "message", "user_created.first_name", "user_created.last_name", "user_created.avatar", "user_created.location", "user_created.telephone", "objet.nom", "objet.marque", "objet.prix_indicatif", "objet.photo", "objet.etat",
      ],
    }));
